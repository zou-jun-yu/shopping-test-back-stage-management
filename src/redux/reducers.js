...
//初始祖先链，只有一个分类对象
const initSelectedCategoryChainNodes = [
  { _id: "0", categoryName: "一级分类列表", parentId: -1, _v: -1 },
];

export const selectedCategoryChainNodes = (
  state = initSelectedCategoryChainNodes,
  action
) => {
  switch (action.type) {
    ...
    case "clickSelectedCategoryChainNode":
      return state.slice(
        0,
        state.indexOf(action.selectedCategoryChainNode) + 1
      );
